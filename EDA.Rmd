...
The first thing to do is to look at the distribution of the categories. The gender is the only nominal category for now, we will change the age category later. 
...
ggplot(cm_data, aes(x=variable, y=value, fill = variable)) +
  geom_boxplot() +
  ggtitle("Distribution plots of the size categories") +
  ylab("Centimeters") +
...
```
 ...
 The boxplots show that each categorie has some outliers but the height category has 2 extreme outliers with no corresponding outliers in the other categories. Since it are only 2 rows and it seems unlikely that these crabs with these porportions exist they will be removed.
...
```
 ...
 A quick nrow shows us that we indeed have removed 2 rows again. The same boxplots as with the length categories are made with the weight categories.
 
```{r}
gram_data <- melt(crab_data[5:8])
ggplot(gram_data, aes(x=variable, y=value, fill=variable)) + 
  geom_boxplot() +
  ggtitle("Distribution of the weight categories") +
  ylab("Grams") +
  #Again x axis title and legend removed due to the values speaking for themselves
  theme(axis.title.x = element_blank(), legend.position = "none")
```
The weight categories also have outliers but they do not seem extreme. Next let's look at the distribution of the age through a histogram. 
...
```
The red line represents the age of 11 from where the growth seems to stagnate in all of the 4 categories. With this in mind 2 groups of crabs are created, aged 1 through 10 months and 11 and above months.
...
Next a pair plot is made with the continuous categories and the points are colored in with the age groups. 

```{r}
ggpairs(crab_data, columns = 2:8, aes(color=Age, alpha=0.3), upper = list(continuous = "points"))
```

The pair plot shows that the young crabs under 11 months colored in red are mostly at the left side of the graph and the crabs which are 11+ months colored in blue are mostly on the right side in the graphs. The middle part does not show a clear line between red and blue which means that it can be harder to predict with machine learning. There seem to be some correlation between the different categories, especially the length compared to the diameter. Let's look deeper into the correlations with a correlation heatmap. 
...
```
The lowest correlation seems to be between the shucked weight and the height, but this is still around 0.84 which suggest they are correlated. The category that seems to be correlating the most is the weight category, not only does it get high correlation scores in the weight categories but also the lenght and diameter seem te be of importance.  

## Conclussion
The data was already very clean but was cleaned further bij removing 2 extreme outliers and 2 zero values. There seems to be some correlation based on pair plots and heatplots. The pair plot seems to show the separation of the 2 age categories up to some degree with a bit of a less defined seperation in the middle. All by all does it seem to be a good dataset to answer the question: "How accurately can you predict the age group of crabs based on physical properties and gender?". 
